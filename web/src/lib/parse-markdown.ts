...
function walk(tokens: Token[], out: RenderableBlock[]): void {
  for (const tok of tokens) {
    switch (tok.type) {
      ...
      case "blockquote": {
        // Use blockquote-with-bold-prefix as the callout convention (see SCHEMA.md).
      ...
      case "space":
        ...
        break;
      default:
        // Anything else (table, def, etc.) gets dropped silently — our schema
        // doesn't use them. Could expose as `unsupported` if we ever care.
        break;
    }
  }
}
  ...
  for (const t of tokens) {
    switch (t.type) {
      ...
      case "br":
        ...
        break;
      case "image":
        // alt text only — we don't render images in profiles
